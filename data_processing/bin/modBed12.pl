...
while(<IN>) {
	...
	if($_ !~ m/pseudo/i){
		...
		if($line[9] eq 2){
			...
			if ($start < 1){
				$start = 1;
				print STDERR "Warning start < 1 for $line[0]:$line[3]\n";
			}
			# print OUT join("\t",$line[0],$line[1]-50,$line[2]+50,$line[3],$line[4],$line[5],$line[1]-50,$line[2]+50,$line[8],$line[9]);
			print OUT join("\t",$line[0],$start,$line[2]+50,$line[3],$line[4],$line[5],$start,$line[2]+50,$line[8],$line[9]);
			print OUT "\t";
			...
			print OUT join("\t",$line[0],$start,$line[2]+50,$line[3],$line[4],$line[5],$start,$line[2]+50,$line[8],$line[9],$line[10]+100,$line[11]);
			print OUT "\n";
		}
	}
}
